...
const ProductDetail = () => {
    ...
    useEffect(() => {
        getAllProductFunction();  // Fetch only once on mount
    }, []);
                ...
                <table className="w-full text-left border border-collapse sm:border-separate border-pink-100 text-pink-400">
                    <thead>
                        <tr>
                            ...
                            <th className="h-12 px-6 text-md border-pink-100 text-slate-700 bg-slate-100 font-bold">Edit</th>
                            <th className="h-12 px-6 text-md border-pink-100 text-slate-700 bg-slate-100 font-bold">Delete</th>
                        </tr>
                    </thead>
                    <tbody>
                        {getAllProduct.map((item, index) => {
                            ...
                            return (
                                <tr key={id} className="text-pink-300">
                                    <td className="h-12 px-6 border-pink-100">{index + 1}.</td>
                                    <td className="h-12 px-6 border-pink-100">
                                        <div className="flex justify-center">
                                            <img className="w-20" src={productImageUrl} alt="" />
                                        </div>
                                    </td>
                                    <td className="h-12 px-6 border-pink-100">{title}</td>
                                    <td className="h-12 px-6 border-pink-100">₹{price}</td>
                                    <td className="h-12 px-6 border-pink-100">{category}</td>
                                    <td className="h-12 px-6 border-pink-100">{date}</td>
                                    <td
                                        onClick={() => navigate(`/updateproduct/${id}`)}
                                        className="h-12 px-6 border-pink-100 text-green-500 cursor-pointer"
                                    >
                                        Edit
                                    </td>
                                    <td
                                        onClick={() => deleteProduct(id)}
                                        className="h-12 px-6 border-pink-100 text-red-500 cursor-pointer"
                                    >
                                        Delete
                                    </td>
                                </tr>
